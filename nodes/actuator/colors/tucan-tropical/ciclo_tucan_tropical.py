#!/usr/bin/env python3
"""
PIXARTEK — Ciclo Tucán Tropical (posiciones guardadas)

HOME (sin mover)       → dark_navy.py
Parada #1:  60,338     → amber.py
Parada #2: 129,148     → green.py
Parada #3: 200,036     → teal.py
...
COLORS_SUBDIR = os.path.join(HERE, "tucan tropical")

# Primer color: se dispensa en HOME sin mover
COLOR_INICIO = os.path.join(COLORS_SUBDIR, "dark_navy.py")

# Paradas con movimiento
PARADAS = [
    (60_338,  os.path.join(COLORS_SUBDIR, "amber.py")),
    (129_148, os.path.join(COLORS_SUBDIR, "green.py")),
    (200_036, os.path.join(COLORS_SUBDIR, "teal.py")),
]
...
class CicloTucanTropical:
    ...
    def correr(self):
        print("=" * 50)
        print("  CICLO TUCÁN TROPICAL — posiciones guardadas")
        print("=" * 50)
