#!/usr/bin/env python3
"""
PIXARTEK — Ciclo Mujer con Sombrero (posiciones guardadas)

HOME (sin mover)       → baby_blue.py
Parada #1:  60,338     → azul_cobalto.py
Parada #2: 129,148     → purple.py
Parada #3: 200,036     → lime_green.py
Parada #4: 263,943     → black.py
...
COLOR_INICIO = os.path.join(HERE, "baby_blue.py")

# Paradas con movimiento
PARADAS = [
    (60_338,  os.path.join(HERE, "azul_cobalto.py")),
    (129_148, os.path.join(HERE, "purple.py")),
    (200_036, os.path.join(HERE, "lime_green.py")),
    (263_943, os.path.join(HERE, "black.py")),
]

# Bursts: inicio→#1: 60338 | #1→#2: 68810 | #2→#3: 70888 | #3→#4: 63907
BURSTS = [PARADAS[0][0]] + [
    PARADAS[i][0] - PARADAS[i-1][0] for i in range(1, len(PARADAS))
]


class CicloMujerSombrero:
    def __init__(self):
    ...
    def correr(self):
        print("=" * 50)
        print("  CICLO MUJER CON SOMBRERO — posiciones guardadas")
        print("=" * 50)
        ...
        self._motor_off()
        print("✓ LIMIT_MAX alcanzado — ciclo completo OK — 5 colores")
        print("=" * 50)
        self._limpiar()
